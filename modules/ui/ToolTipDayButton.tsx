...
export const ToolTipDayButton = ({
    ...
    let textContrast = "sss";
    const colorStepNumber = colorstep as unknown as number;
    if ((Number(colorStepNumber) >= 500 || range_start || range_end) && !range_middle) {
                ...
                <span className="inline-block w-fit test">
                    <DayButton {...buttonProps} day={day} modifiers={modifiers}>
                        <div className="block ">
                            ...
                            <div className={`day-number ${textContrast}`}>{day.date.getDate()}</div>

                            {showPrice && !range_end &&
                                <div className={`day-price ${textContrast}`} style={{ fontSize: '10px', fontWeight: '300', marginTop: '-2px' }}>
                                  ${price}
                                </div>
                            }
                            {range_end &&
                                <div className={`day-price`} style={{ fontSize: '10px', fontWeight: '300', marginTop: '-2px' }}>
                                    ➜
                                </div>
                            }
